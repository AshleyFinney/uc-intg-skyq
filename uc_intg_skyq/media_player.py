...
class SkyQMediaPlayer(MediaPlayerEntity):
    ...
    async def _dispatch_command(self, cmd_id: str, params: dict[str, Any] | None) -> bool:
        ...
        if cmd_id in (media_player.Commands.SHUFFLE, media_player.Commands.REPEAT):
            # Sky Q has no shuffle/repeat — but the Remote 3 transport bar
            # renders these buttons regardless of what we advertise in
            # FEATURES, so accept-and-ignore is the only way to stop the UI
            # from popping a "command failed" toast on press.
